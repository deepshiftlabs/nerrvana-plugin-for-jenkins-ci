...
/**
 * Class which communicates with Nerrvana (by performing API calls and interpreting responses)
 * It is wrapped around Apache HttpClient.
 * 
...
public class HttpCommunicator{
  ...
  /**
   * <p>Calculates 'signature' for the parameters to be sent to Nerrvana.</p>
   * <p>Signature then is appended to the call parameters as last name/value pair</p>
   * @param params list of name/value pairs of the HTTP call
   * @return signature string
   * @throws Exception
   */
  private String calculateSignature(List<NameValuePair> params) throws Exception{
    //1. sort
    ...
    SecretKeySpec secret = new SecretKeySpec(settings.secretkey.getBytes(),"HmacSHA256");
    mac.init(secret);
   ...
   * @throws Exception
   */
  private void traceResponseHeaders(HttpResponse response) throws Exception{
  ...
  private HttpEntity request(String resource,List<NameValuePair> params,boolean bGet) throws Exception{
    ...
    try{
      ...
      client = createClient();
      if(bGet){
        fullUrl = makeUrl(settings.httpurl,resource)+"?"+URLEncodedUtils.format(params, "UTF-8");
      ...
      if(statusCode == 200){
        ...
        sb.append(sline.toString()).append(": ").append(Utils.parseErrorResponse(entity)).append("\n-----END ERROR--\n");
  ...
  /**
   * <p>Adds additional logging to exception occured during HTTP call.</p>
   * @param doc parsed XML of Nerrvana API response 
   * @param e exception occurred during response processing
   ...
   * @throws Exception
   */
  public NerrvanaExecution getExecutionStatus(NerrvanaExecution exec) throws Exception{
    Document doc = null;
    HttpEntity entity = null;
    try{
      List<NameValuePair> params = new ArrayList<NameValuePair>();
      params.add(new BasicNameValuePair("id", exec.id));
  ...
  /**
   * <p>Calls Nerrvana API to create {@link <a href="https://nerrvana.com/docs/test-runs">Nerrvana test run</a>}</p>
   * All function parameters are obtained from @see NerrvanaPluginSettings object, which, in turn
   * obtains them from plugin settings.
   * 
   * @param space_id 
   * @param name
   * @param descr
  ...
  public Testrun createTestrun(
          ...
          boolean bValidation,
          int nodes_count) throws Exception{
    Document doc = null;
    HttpEntity entity = null;
    StringBuilder sb = new StringBuilder();
    ...
    try{
      ...
      params.add(new BasicNameValuePair("executable_file", executable_file));
      params.add(new BasicNameValuePair("validation", bValidation ? "1":"0"));
  ...
  /**
   * Obtains {@link <a href="https://nerrvana.com/docs/spaces">Nerrvana space</a>} information by its name
   * 
   * @param space Nerrvana space name
   * @return
   * @throws Exception
   */
  //This function is not used as we obtain required info through plugin settings
/*
  public SpaceData getSpaceData(String space) throws Exception{
    Document doc = null;
    HttpEntity entity = null;
    try{
      List<NameValuePair> params = new ArrayList<NameValuePair>();
      params.add(new BasicNameValuePair("name", space));
      entity = get("space",params);
      doc = Utils.parseResponse(entity);
      return SpaceData.parseSpaceData(doc);
    }
    catch(Exception e){
      throw handleResponseError(doc,e);
    }
