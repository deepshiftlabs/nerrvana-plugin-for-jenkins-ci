...
public class Logger {
	...
	/**
	 * Logs exception. Also tries to determine if exception has been caused by
	 * some XML failure (unparseable XML or wrong XML) and logs last XML
	 * document as well.
	 * 
	 * @param e
	 *            exception to log
	...
	public static void exception(Throwable e) {
		if (singleInstance != null) {
			...
			if (cls.indexOf("xml") >= 0 && Utils.lastXMLAsString != null
					&& Utils.lastXMLAsString.length() > 0) {
	 ...
	 * 
	 * @param trace
	 *            string to log if current log level == LL_TRACE
	 * @param normal
	 *            string to log if current log level == LL_NORMAL
	 */
